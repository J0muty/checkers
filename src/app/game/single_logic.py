...
async def bot_turn(board: Board, player: str, difficulty: str = "easy") -> Tuple[Board, List[Tuple[int, int]], List[Tuple[int, int]]]:
    if difficulty == "easy":
    ...
    while is_capture:
        ...
        if difficulty == "easy":
            ...
            for d in caps:
                cb = await validate_move(board, end, d, player)
                sc = await minimax(cb, 'white' if player == 'black' else 'black', player, 3, -math.inf, math.inf)
                if sc > best_score:
                    best_score = sc
                    dest = d
                    nb = cb
